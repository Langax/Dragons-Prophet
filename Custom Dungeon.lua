...
local FLAG_BOSS_ONE         = 45002001   -- Boss
local NPC_BOSS_ONE          = 11000185
local BOSS_ONE_AI           = "ny_CrawlerBossAI"

-- ─── Path Two ───────────────────────────────────────────────
local FLAG_PATH_TWO_NPC     = { 45001904, 45001905, 45001906, 45001907 }  -- 
local NPC_BANDIT_ID         = { nil, 10007469, nil, 10006862}  -- Normal Bandit, Healer Goblin, Elite Bandit, Stunning Goblin

local PATH_TWO_KILL_GOAL    = 45

local FLAG_BOSS_TWO         = 45002002
local NPC_BOSS_TWO          = 10016331
...
local State = {
    ...
    Placeholder   = nil,
    Doors         = {},
    Packs         = {},   -- all spawned spiders (path one)
    Boss          = nil,
...
local function ny_SpawnDoor(flag, idx)
    local door = CreateObjByFlag(NPC_DOOR_ID, flag, idx)
...
function ny_SpiderPathOne()
	...
	ScriptMessage( OwnerID() , OwnerID() , EM_ScriptMessageSendType_Room , EM_ClientMessage_Chat , "Strike down 35 Spiders to break through!" , 0 ) 
	ScriptMessage( OwnerID() , OwnerID() , EM_ScriptMessageSendType_Room , EM_ClientMessage_Quest , "Strike down 35 Spiders to break through!" , 0 ) 
    ...
    for i = #State.Packs, 1, -1 do
        ...
        PlayMotionEX(mob, 34000240, 34000240, 0 )
    end
